      ...
      <div class="container mx-auto py-5 flex justify-between">
        <div class="flex gap-10">
          ...
          <div class="relative hidden lg:block">
            <input
              type="text"
              class="input outline-0 px-4 rounded-md w-[320px] h-full"
              placeholder="Bạn cần tìm kiếm gì?"
            />
            <div class="absolute top-2 right-4 text-gray-500">
        ...
        <div class="text-white flex gap-3">
          <NuxtLink
            ...
            class="flex gap-2 w-fit p-2 rounded-md px-3 cursor-pointer"
          >
            <i class="bi bi-journal-text"></i>
            <div class="hidden md:block">Tra cứu đơn hàng</div>
          </NuxtLink>
          <NuxtLink
            to="/don-hang"
            class="flex gap-2 w-fit p-2 rounded-md px-3 cursor-pointer"
          >
            <el-badge :value="cart.cart.length" class="item" :show-zero="false">
              <i class="bi bi-cart-dash"></i>
            </el-badge>
            <div class="hidden md:block">Giỏ hàng</div>
          </NuxtLink>

          <el-dropdown placement="bottom-end" @handleClose="() => {}">
            <div
              class="flex gap-2 bg-red-700 w-fit p-2 rounded-md px-3 cursor-pointer text-white text-base outline-none"
            >
              <i class="bi bi-person"></i>
              <div v-if="!auth.user">Đăng nhập</div>
              <div v-else>{{ auth.user.name }}</div>
            </div>
            <template #dropdown>
              <div class="p-4 min-w-[320px]">
                <div class="text-base flex gap-3">
                  <span class="w-[20px]">
                    <svg
                      width="18"
                      height="17"
                  ...
                  <div>
                    Xin chào,
                    {{ auth.user ? auth.user.name : "vui lòng đăng nhập" }}
                  </div>
                </div>
                <div
                  class="flex gap-3 mt-2 text-base text-center"
                  v-if="!auth.user"
                >
                  <div
                    class="p-2 px-1 bg-zinc-900 text-white w-full rounded-sm leading-6 cursor-pointer"
                    @click="useEmit('show-login')"
                  >
                    ĐĂNG NHẬP
                  </div>
                  <div
                    class="w-full border border-zinc-900 p-2 rounded-sm leading-6 cursor-pointer"
                    @click="useEmit('show-register')"
                  >
                    ĐĂNG KÝ
                  </div>
                </div>
                <div v-else>
                  <div class="mt-4 pt-4 border-t">
                    <NuxtLink
                      to="/nguoi-dung/ho-so"
                      class="flex gap-4 text-base mb-4"
                    >
                      <i class="bi bi-person-circle w-[20px]"></i>
                      <div>Hồ sơ</div>
                    </NuxtLink>
                    <NuxtLink
                  ...
                  <div class="mt-3 pt-3 border-t">
                    <div class="flex gap-4 text-base" @click="auth.signOut()">
                      <i class="bi bi-box-arrow-right w-[20px]"></i>
                      <div>Đăng xuất</div>
                    </div>
                  </div>
                </div>
